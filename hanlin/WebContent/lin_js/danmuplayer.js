...
(function($) {
	...
	var DanmuPlayer = function(element, options) {
		...
		danmu_video = videojs("#danmu_video", {
			...
			$(".vjs-control-bar").append('<input  role="botton" type="textarea" id="danmu_text" maxlength="30"  max=300 />'); // ->
																												// button
...
function send_danmu() {
	...
	if(getStrLen(text)>30){
		ShowTipAlwaysInTheMiddle("弹幕字数不能超过30个哦！",'danger');
	}else{
