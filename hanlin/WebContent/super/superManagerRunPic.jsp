...
<body>
	...
	<div id="middle_part" class="container-fluid">
		<div class="row">
			...
			<div id="content_part" class="col-md-9">
				<div class="row">
					<div class="col-md-8 col-xs-12" style="margin-top:15px;margin-bottom:15px;">
						<strong>当前位置:</strong><a href="#">管理滚屏</a>
					</div>
				</div>
				<hr>
				<div class="row">
					<div class="col-md-8 col-md-offset-1">
						<h4>上传首页滚屏</h4>
						<form action="<%=basePath%>super_uploadRunpic.do" method="post"
							enctype="multipart/form-data">
							<p>
								滚屏描述：
								<textarea name="disc" class="form-control" rows="3" id="disc_textarea" pattern="^([\u4e00-\u9fa5]|[0-9_a-zA-Z]){1,100}$" required></textarea>
							</p>
							<p>
								滚屏：<input type="file" id="userHead" name="image" required/><img
									src="<%=basePath%>/head/defaultcourse.jpg"
									width="300px" height="180px" id="img0" />
								<script>
									$(function() {
										$("#userHead")
												.change(
							...
							<p>
								<input type="submit" class="btn btn-default" value="上传" /><span style="color:red;"><s:property value="#request.tip"/></span>
							</p>
						</form>
					</div>
				</div>
			...
			<div class="row">
				...
				<s:if test="#session.runpics.size()==0">
					<h4 style="color: red;">您还没有上传任何滚屏哦！</h4>
				</s:if>
				<s:else>
					<div class="col-md-8 col-md-offset-1">
					    <h4>管理滚屏</h4>
						<div class="table-responsive">
							<table class="table table-bordered table-hover">
								...
								<s:iterator value="#session.runpics">
									<tr>
										<td><s:property value="runpicId" /></td>
										<td><a href="#"><s:property value="disc" /></a></td>
										<td><a target="_blank" href="../img/<s:property value="runpicPath" />"><img width="100px" height="80px"
											src='../img/<s:property value="runpicPath" />' /></a></td>
										<td><s:property value="time" /></td>
										<td><a class="btn btn-default btn-sm" role="button"
											href="<%=basePath%>super_deleteRunpic.do?runpicId=<s:property value='runpicId'/>">删除</a></td>
									</tr>
								</s:iterator>
							</table>
						</div>
	...
	<div id="myModal" class="modal fade" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog modal-sm">
			<div class="modal-content">
				...
				<form action="super_sendMessageToTea.do" method="post"
					onSubmit="return show();">
					<input name="message.senderId" type="hidden"
						value="<s:property value='#session.loginUser.getUserId()'/>" /> <input
						id="recieverId_message" name="message.recieverId" type="hidden"
						value="" />
					<p>
						标题：<br> <input type="text" class="form-control"
							name="message.title" placeholder="私信内容描述" />
					</p>
					<p>
						内容：<br>
						<textarea name="message.content" row="3" col="10">请输入私信内容</textarea>
					</p>
					<p>
	...
	<script type="text/javascript">
	disc_textarea.oninput=function(){
		disc_textarea.setCustomValidity("");
	}
	disc_textarea.oninvalid=function(){
		disc_textarea.setCustomValidity("请输入100字以内的滚屏描述!");
	}
		function show() {
